...
if (!$_SESSION['AMP_user']) {
        die('Accesso non effettuato! Accedi alla dashboard di FreePBX prima di aprire questa pagina...');
}
...
function get_device_info($ua) {
  ...
  switch ($ua_arr[0]) {
    ...
    case "A510":
      $mod_firm_arr = preg_split("/[\s]/", $ua_arr[1]);
      $device_info = ["brand" => "Gigaset", "model" => $ua_arr[0], "firmware" => $mod_firm_arr[0]];
      break;
    case "Cisco-CP7940G":
      $mod_firm_arr = preg_split("/[\s]/", $ua_arr[1]);
      $device_info = ["brand" => "Cisco", "model" => "CP7940G", "firmware" => $mod_firm_arr[0]];
      break;
	case "Cisco-CP-6851-3PCC":
      $mod_firm_arr = preg_split("/[\s]/", $ua_arr[1]);
      $device_info = ["brand" => "Cisco", "model" => "CP-6851-3PCC", "firmware" => $mod_firm_arr[0]];
      break;
	case "Alcatel":
      $mod_firm_arr = preg_split("/[\s]/", $ua_arr[1]);
      $device_info = ["brand" => "Alcatel", "model" => "$mod_firm_arr[0]", "firmware" => $mod_firm_arr[1]];
      break;
    ...
    default:
      ...
      // Jitsi on Windows does not have a split character.
        ...
        $device_info = ["brand" => "Sconosciuto", "model" => "", "firmware" => ""];
      }
  }
  return $device_info;
}
